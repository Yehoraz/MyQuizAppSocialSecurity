...
public class SecurityConfigurer extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.antMatcher("/**").authorizeRequests().antMatchers("/player/register", "/login**", "/player/logoutSuccess")
				.permitAll().antMatchers("/creator**").hasRole("CREATOR").antMatchers("/admin**").hasRole("ADMIN")
				.antMatchers("/manager**").hasRole("MANAGER").antMatchers("/player**").hasRole("PLAYER").and()
				.formLogin().disable().logout().clearAuthentication(true).invalidateHttpSession(true)
				.logoutUrl("/logout").logoutSuccessUrl("http://localhost:9090/player/logoutSuccess").permitAll().and()
				.csrf().ignoringAntMatchers("/logout**").csrfTokenRepository(cookieCsrfTokenRepository).and()
				.addFilterBefore(ssoFilter(), BasicAuthenticationFilter.class).exceptionHandling()
...
//	}
	...
	@Bean
	public FilterRegistrationBean<OAuth2ClientContextFilter> oauth2FilterRegistration(
